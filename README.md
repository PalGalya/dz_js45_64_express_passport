# Express + Passport + MongoDB Atlas# Express Server з Passport авторизацією та MongoDB Atlas# Express Server з Passport авторизацією та MongoDB Atlas# Express Server з Passport авторізацією та MongoDB Atlas# Express Server з Passport авторизацією
...
Інтеграція Passport.js для локальної авторизації користувачів.

**Домашнє завдання 65.1:** Інтеграція MongoDB Atlas з існуючим сервером Express.

### ДЗ 65.1 - MongoDB Atlas інтеграція

Підключення до MongoDB Atlas та реалізація операцій читання даних.Навчальний проєкт, який демонструє інтеграцію Express сервера з:
...
## 🎯 Що реалізовано- **MongoDB Atlas** для зберігання та читання даних



### Авторизація (Passport.js)- **Express Session** для управління сесіями користувачів## 📋 Технічні завдання

- Реєстрація нових користувачів

- Вхід/вихід з системи- **Celebrate + Joi** для валідації даних

- Збереження сесій через `express-session`
...
- Колекція `products` з 5 тестовими продуктами

- REST API для роботи з продуктами### Технології (ДЗ 65.1)

- Автоматичне додавання тестових даних (seed)

- Веб-сторінка для відображення продуктів- ✅ Node.js та Express.js- **MongoDB Atlas** для зберігання та читання даних
...
- **MongoDB** (офіційний драйвер)

- **express-session** (сесії з httpOnly cookies)### Підключення MongoDB Atlas- **Celebrate + Joi** для валідації даних

- **Celebrate + Joi** (валідація)

- **dotenv** (змінні оточення)- ✅ Налаштовано з'єднання з базою даних у Atlas
...
## 🚀 Швидкий старт- ✅ Graceful shutdown при зупинці сервера



### 1. Клонування та встановлення## ✅ Виконані вимоги



```bash### Операція читання даних

git clone https://github.com/PalGalya/dz_js45_64_express_passport.git

cd dz_js45_64_express_passport- ✅ Створено маршрути для отримання даних з MongoDB- Node.js та Express.js

git checkout dz_js46_65_mongo_express_integration

npm install- ✅ Реалізовано логіку запиту даних з колекції `products`
...
Файл `.env` вже включений у репозиторій з робочими даними:

- ✅ Node.js та Express.js- Node.js та Express.js

```env

MONGODB_URI=mongodb+srv://adminUser:Pal.Daniil.2011@cluster0.jwiqlkt.mongodb.net/expressApp?retryWrites=true&w=majority&appName=Cluster0### Авторизація (ДЗ 64.1)

SESSION_SECRET=my-super-secret-key-change-in-production

PORT=3000- ✅ Passport.js з LocalStrategy (email + пароль)- ✅ MongoDB Atlas (хмарна база даних)
...
npm start- ✅ Express Session з httpOnly cookies### 2. Підключення MongoDB Atlas- Passport.js для авторизації
...
| POST | `/auth/login` | Вхід в систему | ❌ |```### 3. Операція читання даних- ✅ Створення маршрутів для реєстрації, входу та виходу користувачів

| POST | `/auth/logout` | Вихід | ✅ |

| GET | `/auth/me` | Поточний користувач | ✅ |

| GET | `/auth/protected` | Тестовий захищений маршрут | ✅ |
...
- **`/products.html`** - сторінка з каталогом продуктів
...
curl -X POST http://localhost:3000/auth/login \- ✅ Захищені маршрути з middleware

  -H "Content-Type: application/json" \

  -d '{"email":"admin@example.com","password":"password123"}' \Файл `.env` вже включений у репозиторій для зручності перевірки викладачем:
...
### Отримання продуктів```env



```bashMONGODB_URI=mongodb+srv://<username>:<password>@<cluster>.mongodb.net/expressApp?retryWrites=true&w=majority## 🚀 Встановлення та запуск- ✅ Збереження ідентифікатора сесії у cookies з `httpOnly` та `secure`
